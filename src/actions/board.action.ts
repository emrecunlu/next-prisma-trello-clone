...
export async function updateOrder(reorderBoardDto: ReoderBoardDto) {
  return tryCatch(() => {
    return prisma.$transaction(async (tx): Promise<ActionResult> => {
      ...
      const { error, data } = await updateOrderSchema.safeParseAsync(
        reorderBoardDto
      );
      ...
      const destination = await tx.board.findFirst({
        where: {
          userId: session.user?.id,
          order: data.order,
        },
      ...
      if (!destination) throw new Error(t("errors.boardNotFound"));

      const boards = await tx.board.findMany({
        where: { userId: session.user?.id },
      });

      const sourceIndex = boards.findIndex((x) => x.id === board.id);
      const destinationIndex = boards.findIndex((x) => x.id === destination.id);

      const [removed] = boards.splice(sourceIndex, 1);
      boards.splice(destinationIndex, 0, removed);
